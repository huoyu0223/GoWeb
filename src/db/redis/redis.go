...
func NewRedisPool(ip string, port int, password string, db int, poolSize int) error {
	...
	if err != nil {
		logrus.Error("new redis pool error:", err)
		return err
	}
...
func Set(key, value string) bool {
	...
	if err != nil {
		logrus.Error("redis set error key:", key, " value:", value, " error:", err)
		return false
	}
	return result == "OK"
}
...
func SetEX(key, value string, ex time.Duration) bool {
	...
	if err != nil {
		logrus.Error("redis setex error key:", key, " value:", value, " error:", err)
		return false
	}
	return result == "OK"
}
...
func Get(key string) (bool, string) {
	...
	if err != nil {
		logrus.Error("redis get error key:", key, " error:", err)
		return false, ""
	}
	return true, result
}
...
func GetSet(key, value string) (bool, string) {
	...
	if err != nil {
		logrus.Error("redis get set error key:", key, " value:", value, " error:", err)
		return false, ""
	}
...
func Incr(key string) int64 {
	...
	if err != nil {
		logrus.Error("redis incr error key:", key, " error:", err)
	}
	return val
}
...
func IncrBy(key string, incr int64) int64 {
	...
	if err != nil {
		logrus.Error("redis IncrBy error key:", key, " value:", incr, " error:", err)
	}
	return val
}
...
func IncrByFloat(key string, incrFloat float64) float64 {
	...
	if err != nil {
		logrus.Error("redis IncrByFloat error key:", key, " value:", incrFloat, " error:", err)
	}
	return val
}
...
func Decr(key string) int64 {
	...
	if err != nil {
		logrus.Error("redis Decr error key:", key, " error:", err)
	}
	return val
}
...
func DecrBy(key string, incr int64) int64 {
	...
	if err != nil {
		logrus.Error("redis DecrBy error key:", key, " error:", err)
	}
	return val
}
...
func Del(key string) bool {
	...
	if err != nil {
		logrus.Error("redis Del error key:", key, " error:", err)
		return false
	}
	return result == 1
}
...
func Expire(key string, ex time.Duration) bool {
	...
	if err != nil {
		logrus.Error("redis Expire error key:", key, " error:", err)
		return false
	}
	return result
}
...
func LPush(key string, date ...interface{}) int64 {
	...
	if err != nil {
		logrus.Error("redis LPush error key:", key, " error:", err)
	}
	return result
}
...
func RPush(key string, date ...interface{}) int64 {
	...
	if err != nil {
		logrus.Error("redis RPush error key:", key, " error:", err)
	}
	return result
}
...
func LPop(key string) (bool, string) {
	...
	if err != nil {
		logrus.Error("redis LPop error key:", key, " error:", err)
		return false, ""
	}
	return true, val
}
...
func RPop(key string) (bool, string) {
	...
	if err != nil {
		logrus.Error("redis RPop error key:", key, " error:", err)
		return false, ""
	}
	return true, val
}
...
func LIndex(key string, index int64) (bool, string) {
	...
	if err != nil {
		logrus.Error("redis LIndex error key:", key, " error:", err)
		return false, ""
	}
	return true, val
}
...
func LLen(key string) int64 {
	...
	if err != nil {
		logrus.Error("redis LLen error key:", key, " error:", err)
	}
	return val
}
...
func LRange(key string, start, stop int64) []string {
	...
	if err != nil {
		logrus.Error("redis LRange error key:", key, " error:", err)
	}
	return vales
}
...
func LRem(key string, count int64, data interface{}) bool {
	...
	if err != nil {
		logrus.Error("redis LRem error key:", key, " error:", err)
	}
	return true
}
...
func LInsert(key string, pivot int64, data interface{}) bool {
	...
	if err != nil {
		logrus.Error("redis LInsert error key:", key, " error:", err)
		return false
	}
	return true
}
...
func SAdd(key string, data ...interface{}) bool {
	...
	if err != nil {
		logrus.Error("redis SAdd error key:", key, " error:", err)
		return false
	}
	return true
}
...
func SCard(key string) int64 {
	...
	if err != nil {
		logrus.Error("redis SCard error key:", key, " error:", err)
	}
	return size
}
...
func SIsMember(key string, data interface{}) bool {
	...
	if err != nil {
		logrus.Error("redis SIsMember error key:", key, " error:", err)
	}
	return ok
}
...
func SMembers(key string) []string {
	...
	if err != nil {
		logrus.Error("redis SMembers error key:", key, " error:", err)
	}
	return es
}
...
func SRem(key string, data ...interface{}) bool {
	...
	if err != nil {
		logrus.Error("redis SRem error key:", key, " error:", err)
		return false
	}
	return true
}
...
func SPopN(key string, count int64) []string {
	...
	if err != nil {
		logrus.Error("redis SPopN error key:", key, " error:", err)
	}
	return vales
}
...
func HSet(key, field, value string) bool {
	...
	if err != nil {
		logrus.Error("redis HSet error key:", key, " error:", err)
		return false
	}
	return true
}
...
func HGet(key, field string) string {
	...
	if err != nil {
		logrus.Error("redis HGet error key:", key, " error:", err)
	}
	return val
}
...
func HMGet(key string, fields ...string) []interface{} {
	...
	if err != nil {
		logrus.Error("redis HMGet error key:", key, " error:", err)
	}
	return vales
}
...
func HGetAll(key string) map[string]string {
	...
	if err != nil {
		logrus.Error("redis HGetAll error key:", key, " error:", err)
	}
	return data
}
...
func HKeys(key string) []string {
	...
	if err != nil {
		logrus.Error("redis HKeys error key:", key, " error:", err)
	}
	return fields
}
...
func HLen(key string) int64 {
	...
	if err != nil {
		logrus.Error("redis HLen error key:", key, " error:", err)
	}
	return size
}
...
func HMSet(key string, data map[string]interface{}) bool {
	...
	if err != nil {
		logrus.Error("redis HMSet error key:", key, " error:", err)
		return false
	}
	return result
}
...
func HSetNX(key, field string, value interface{}) bool {
	...
	if err != nil {
		logrus.Error("redis HSetNX error key:", key, " error:", err)
		return false
	}
	return result
}
...
func HDel(key string, fields ...string) bool {
	...
	if err != nil {
		logrus.Error("redis HDel error key:", key, " error:", err)
		return false
	}
	return true
}
...
func HExists(key, field string) bool {
	...
	if err != nil {
		logrus.Error("redis HExists error key:", key, " error:", err)
		return false
	}
	return result
}
